...
/**
 * Cell editor for computed and default cells. This editor enables to edit the
 * definition of an expression if the edited value is an instance of
...
public class JeksCellEditor extends DefaultCellEditor
{
     ...
     * 
     * @param parser
     *            the parser used to parse and compile the cells starting with
    ...
    public boolean stopCellEditing()
    {
        ...
        cellValue = getModelValue(
                data,
                table.getModel(),
                new JeksCell(table.getName(),
                             table.getEditingRow(),
    ...
    /**
     * Returns the component used to edit cell values. Overrides
     * <code>getTableCellEditorComponent ()</code> to use the value returned by
     * <code>getEditedValue (value)</code> in the editor instead of the table
     * model <code>value</code> itself.
     */
     ...
     */
    public Object getModelValue(String data, TableModel model,
            JeksCell editingCell)
    {
        try
        {
            if (parser != null)
            {
                ...
                if (modelValue instanceof JeksExpression)
                    try
                    {
                        ((JeksExpression) modelValue).checkCircularity(state,
                                model, editingCell);
                    } catch (CircularityException ex)
                    {
                        JeksExpressionSyntax syntax = (JeksExpressionSyntax) parser
                                .getSyntax();
                        if (JOptionPane
                                .showConfirmDialog(
                                        null,
                    ...
                    {
                        ((JeksExpression) modelValue).invalidateValue();
                    }
                return modelValue;
            } else
                return data;
        } catch (CompilationException ex)
        {
            ...
            // ((JTextField)getComponent ()).setCaretPosition
            // (ex.getCharacterIndex
            // ());
            return null;
        }
    }
