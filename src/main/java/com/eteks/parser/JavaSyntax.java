...
/**
 * Syntax used by parsers for functions and expressions written in Java. This
 * syntax supports the Java unary and binary operators and the static methods of
 * the <code>java.lang.Math</code> class. The multi parameters methods of
 * <code>java.lang.Math</code> are implemented as Java functions implementing
 * the <code>Function</code> interface and added to the syntax.<br>
 ...
 * <LI>Constants : <code>Math.PI Math.E</code>. If the instance of
 * <code>JavaSyntax</code> is created with its parameter
 * <code>extendedSyntax</code> equal to <code>true</code>, <code>true</code> and
 * <code>false</code> are also supported.</LI>
 * <LI>Functions :
 * <UL>
 ...
 * <UL>
 * <LI>Primary operators : <code>. [ ] new</code></LI>
 * <LI>Unary operators : <code>++ --</code> and casts</LI>
 * <LI>Binary operators : <code>instanceof</code></LI>
 * <LI>Affectation operators : only <code>=</code> returned by
 * <code>getAssignmentOperator ()</code> is used.</LI>
 * </UL>
 * The literals may be any Java literal number. If <code>extendedSyntax</code>
 * is equal to <code>true</code>, quoted characters and strings are also
 * supported as literals.<br>
 * The parsed identifiers (function name and its parameters) may be any Java
 * identifier containing letters, digits, the characters $, _ <b>or</b> .
 * (dots). The first character of an identifier must be a letter, the characters
 * $ or _ (not a dot). No type checking is done during the parsing of functions
 * and expressions but using a <code>WrapperInterpreter</code> interpreter
 * enables to fill this lack at runtime.
 * 
 * @version 1.0.2
...
public class JavaSyntax extends AbstractSyntax
{
    ...
    /**
     * Creates a Java syntax, that doesn't use an extended syntax (no boolean
     ...
     * 
     * @param extendedSyntax
     *            if equal to <code>true</code>, this syntax supports the
     *            boolean constants <code>true</code> and <code>false</code> and
    ...
    public JavaSyntax(boolean extendedSyntax)
    {
        ...
        // As types are used only at the time of interpretation, Math.round ()
        // and
        // Math.rint ()
        ...
        // Add all the other Math methods of JDK 1.0 (not including toDegrees ()
        // and
        // toRadians ())
     ...
     * number or if this instance supports an extended syntax, a string or a
     * character.
     * 
     * @param expression
     *            the string to parse.
     ...
     *         <code>String</code> is returned and if the literal is a character
     ...
     * string buffer <code>extractedNumber</code>. The parsed number may be any
     * Java literal number (hexadecimal, octal or decimal number) possibly
     * followed by a type letter d, f or l (upper case or lower case).
     * 
     * @param expression
     *            the string to parse.
            ...
            StringBuffer extractedNumber)
    {
        ...
        if (expression.length() > 3
                ...
                && ((c = expression.charAt(2)) >= '0' && c <= '9' || c >= 'a'
                        && c <= 'f' || c >= 'A' && c <= 'F'))
        {
            // Parse hexadecimal number
            extractedNumber.append(c);
            for (int index = 3; index < expression.length()
                    && ((c = expression.charAt(index)) >= '0' && c <= '9'
                            || c >= 'a' && c <= 'f' || c >= 'A' && c <= 'F'); index++)
                extractedNumber.append(c);
            literal = new Long(Long.parseLong(extractedNumber.toString()
                    .substring(2), 16));
        } else if (expression.length() > 2 && expression.charAt(0) == '0'
                && (c = expression.charAt(1)) >= '0' && c <= '7')
        {
            // Parse octal number
            int index;
            for (index = 2; index < expression.length()
                    && (c = expression.charAt(index)) >= '0' && c <= '7'; index++)
                ;
        ...
        if (literal != null && expression.length() > extractedNumber.length())
            ...
            } else if ((c == 'l' || c == 'L') && literal instanceof Long)
    ...
    /**
     * Extracts the quoted character or string from the expression
     * <code>expression</code> in the string buffer <code>extractedString</code>
     * . The parsed string may be any Java literal character or string possibly
     * using escape sequence (\", \t, \u00e9,...).
     * 
     * @param expression
     *            the string to parse.
     * @param extractedString
     *            the quoted char or string extracted from
     *            <code>expression</code>.
     * @return <code>null</code> if <code>expression</code> doesn't start with a
     *         quoted character or string or an instance of
     *         <code>Character</code> or <code>String</code> according to the
     *         parsed character or string.
     */
        ...
        if (expression.length() >= 2)
            if (expression.charAt(0) == '\'')
            {
                if (expression.length() > 2)
                {
                    ...
                    if (c != -1 && expression.length() >= 2 + buffer.length()
            ...
            } else if (expression.charAt(0) == '\"')
            {
                ...
                StringBuffer buffer = new StringBuffer();
                int index;
                for (index = 1; index < expression.length()
                        && expression.charAt(index) != '\"'; index += buffer
    ...
    protected int getCharacter(String expression, int index,
            StringBuffer extractedString)
    {
            ...
            switch (expression.charAt(index + 1))
            {
                ...
                default:
                    if ((c = expression.charAt(index + 1)) >= '0' && c <= '7')
                    {
                        ...
                        int i = 2;
                        while (i < 4 && index + i < expression.length()
                        ...
                        {
                            // Numbers with 3 octal digits may start only with 0
                            // 1 2 or 3
                            extractedString.append(expression.substring(index,
                                    index + i));
                            return Integer.parseInt(
                                    expression.substring(index + 1, index + i),
                                    8);
                        }
                        ...
                        if (expression.charAt(index + 1) == 'u')
                        {
                            ...
                            extractedString.append(expression.substring(index,
                                    index + 6));
                            return Integer.parseInt(
                                    expression.substring(index + 2, index + 6),
                                    16);
                        }
            }
     ...
     * correctly written identifier usable as the name of a function or a
     * paramater.
     * 
     * @param identifier
     *            the string to test.
    ...
    /**
     * Returns <code>true</code> if the string <code>identifier</code> is a Java
     * identifier. An identifier of the syntax <code>JavaSyntax</code> may
     * contain letters, digits, the characters $, _ <b>or</b> . (dots). The
     * first character of <code>identifier</code> must be a letter, the
     * characters $ or _ (not a dot). The dot allows to emulate static fields
     * and methods of Java.
     * 
     * @param identifier
     *            the string to test.
     * @return <code>true</code> if <code>identifier</code> is a Java
     *         identifier.
     */
    ...
    private static class FunctionRemain implements Function
    {
                ...
                Object[] parametersValue)
        {
            if (parametersValue[0] instanceof Number
                    && parametersValue[1] instanceof Number)
                return new Double(Math.IEEEremainder(
                        ((Number) parametersValue[0]).doubleValue(),
                        ((Number) parametersValue[1]).doubleValue()));
            else
                throw new IllegalArgumentException("Can compute only numbers");
    ...
    private static class FunctionMax implements Function
    {
                ...
                Object[] parametersValue)
        {
            if (interpreter instanceof MathMLInterpreter)
                return "<apply>\n<max/>\n" + parametersValue[0]
                        + parametersValue[1] + "</apply>\n";
            else if (parametersValue[0] instanceof Number
                    && parametersValue[1] instanceof Number)
            {
                if (interpreter instanceof WrapperInterpreter
                        && (parametersValue[0] instanceof Integer || parametersValue[0] instanceof Long)
                        && (parametersValue[1] instanceof Integer || parametersValue[1] instanceof Long))
                    return new Long(Math.max(
                            ((Number) parametersValue[0]).longValue(),
                            ((Number) parametersValue[1]).longValue()));
                else
    ...
    private static class FunctionPow implements Function
    {
                ...
                Object[] parametersValue)
        {
            ...
            return interpreter.getBinaryOperatorValue(OPERATOR_POWER,
                    parametersValue[0], parametersValue[1]);
        }
    };
