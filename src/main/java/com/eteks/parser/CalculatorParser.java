...
/**
 * Parser able to compute constant expressions operating unary and binary
 * operators and common functions on numbers. This is the kind of expressions
 * often managed in calculators.<br>
 * The two <code>computeExpression ()</code> methods of this class allow to
 * parse and compute the value of an expression :
 * <UL>
 * <LI><code>double computeExpression (String expressionDefinition)</code> is
 * the easiest method to compute values of type <code>double</code>.</LI>
 * <LI>
 * <code>Object computeExpression (String expressionDefinition, Interpreter interpreter)</code>
 * allows to choose the interpreter with which the numbers and the operators and
 * common functions of <code>Syntax</code> will be computed.</LI>
 * </UL>
 ...
 * <UL>
 * <LI>literals (numbers) accepted by the <code>getLiteral ()</code> method of
 * the syntax</LI>
 ...
 * <code>getCommonFunctionKey ()</code> method of the syntax.</LI></LI>
 * </UL>
 * The binary operators recognized by this parser are infixed, unary operators
 * and common functions are prefixed. Some parts of the expression may be
 * bracketed between the characters returned by
 * <code>getOpeningBracket ()</code> and <code>getClosingBracket ()</code>
 * methods. White spaces returned by the <code>getWhiteSpaceCharacters ()</code>
 * method of the syntax may be used anywhere in the expression.
 * 
 * <P>
 ...
 * 
 * <PRE>
 * // Create a parser with the default syntax <code>DefaultSyntax</code>
 ...
 * double doubleResult5 = parser.<b>computeExpression</b> ("cos (3))");
 * </PRE>
 * 
 * </BLOCKQUOTE>
 * 
 ...
 * <UL>
 * <LI>
 * <code>Object getLiteral (String expression, StringBuffer extractedLiteral)</code>
 * </LI>
 ...
 * <LI><code>boolean isShortSyntax ()</code></LI>
 * </UL>
 * Short cuts of a syntax supports only calls to common function without
 * brackets around their parameter.<br>
...
{
    // Default Syntax and Interpreter aren't assigned at static initialization
    // time
    // to avoid to load automatically the classes DefaultSyntax and
    // DoubleInterpreter
     ...
     * <code>expressionDefinition</code>. <code>expressionDefinition</code> may
     * contain numbers, unary and binary operators, common functions and
     * brackets, according to the syntax of this parser.<br>
     * An interpreter of <code>DoubleInterpreter</code> class is used internally
     * to compute values of type <code>Double</code> and then return a value of
     * type <code>double</code>.
     * 
     * @param expressionDefinition
     *            the constant expression to compute.
     * @return the result of the expression.
     * @throws CompilationException
     *             if the syntax of the expression is incorrect or if some
     *             elements (numbers, operators, functions,...) of the
     *             expression are not accepted by the syntax of the parser.
     * @see com.eteks.parser.DoubleInterpreter
     * @see #computeExpression(String, Interpreter)
     */
    public double computeExpression(String expressionDefinition)
            throws CompilationException
    {
        if (defaultInterpreter == null)
            defaultInterpreter = new DoubleInterpreter();

        Object result = computeExpression(expressionDefinition,
                defaultInterpreter);
     ...
     * Returns the result of the constant expression
     * <code>expressionDefinition</code>. <code>expressionDefinition</code> may
     * contain numbers, unary and binary operators, common functionss and
     * brackets, according to the syntax of this parser.
     * 
     * @param expressionDefinition
     *            the constant expression to compute.
     * @param interpreter
     *            the runtime interpreter used to compute the operations of
     *            <code>Syntax</code>.
     * @return the result of the expression. The returned object may be of any
     *         class (<code>Double</code>, <code>String</code>, ...) according
     *         to <code>interpreter</code> implementation.
     * @throws CompilationException
     *             if the syntax of the expression is incorrect or if some
     *             elements (literals, constants, operators, functions,...) of
     *             the expression are not accepted by the syntax of the parser.
     */
     ...
     * <code>expressionDefinition</code> beginning at the character index
     * <code>definitionIndex</code>.
     * 
     * @param expressionDefinition
     *            the string to parse.
     ...
     * @throws CompilationException
     *             if this method doesn't recognize any calculator's lexical at
            ...
            int definitionIndex, Object parserData) throws CompilationException
    {
        ...
        while (definitionIndex < expressionDefinition.length()
                ...
                && syntax.getWhiteSpaceCharacters().indexOf(
                        expressionDefinition.charAt(definitionIndex)) != -1)
            extractedLexical.append(expressionDefinition
        ...
        // v1.0.1 Moved getLiteral () to the end to give an higher priority for
        // operators
     ...
     * <code>lexical</code> and <code>expressionStack</code> are used to compute
     * the current parsed expression.
     * 
     * @param graphNode
            ...
            throws CompilationException
    {
        ...
        executeTransition(transitionGraph[lexical.getCode()][graphNode],
                definitionIndex, lexical, expressionStack);
    ...
    { 0, 0, 0 }, // Nothing to do
            { ADD_LITERAL, 0, 0 },
            { ADD_UNARY_OP, 0, ADD_UNARY_OP }, // v1.0.2 : Added ADD_UNARY_OP in
                                               // N_C_FCT column
            { 0, ADD_BINARY_OP, 0 },
            { ADD_COMMON_FCT, 0, 0 },
    ...
    private void executeTransition(byte transition, int definitionIndex,
            Lexical lexical, Stack expressionStack) throws CompilationException
    {
        switch (transition)
        {
            case ADD_BINARY_OP:
    ...
    private void addCommonFunction(Lexical commonFunctionLexical,
            Stack expressionStack)
    {
        expressionStack.push(new ElemCal(PRIORITY_COMMON_FUNCTION,
                LEXICAL_COMMON_FUNCTION, commonFunctionLexical.getValue()));
    }
            ...
            Stack expressionStack)
    {
        Object unaryOperatorKey = unaryOperatorLexical.getValue() != null ? unaryOperatorLexical
                ...
                .getExtractedString());
        expressionStack.push(new ElemCal(PRIORITY_COMMON_FUNCTION,
                LEXICAL_UNARY_OPERATOR, unaryOperatorKey));
    }
    ...
    private static class ElemCal
    {
        ...
        public ElemCal(int priority, int operatorType,
                Object operatorKeyOrOperand)
        {
    ...
    private static class StackCal extends Stack
    {
        ...
        public Object pop()
        {
            ...
            if (!empty() && topElement.operatorKeyOrOperand != null)
            {
                ...
                if (element.operatorKeyOrOperand == null)
                    ...
                    switch (element.operatorType)
                    {
                        case LEXICAL_UNARY_OPERATOR:
                            element.operatorKeyOrOperand = interpreter
                                    .getUnaryOperatorValue(
                                            element.operatorKeyOrOperand,
        ...
        public Object push(Object item)
        {
            ...
            if (element.operatorType == LEXICAL_LITERAL)
                element.operatorKeyOrOperand = interpreter
                        .getLiteralValue(element.operatorKeyOrOperand);
