...
/**
 * Parser able to compile functions with parameters of the form <i>function
 * (list of parameters) = expression operating on parameters</i>. The
 * <i>expression</i> may contain unary or binary operators, conditions, common
 * functions or other functions operating on literals, constants and paramaters
 * of the function.<br>
 * A parser is associated to a syntax, instance of the <code>Syntax</code>
 * interface that describes the different lexicals used by the parser and their
 * type.<br>
 * The <code>CompiledFunction compileFunction (String functionDefinition)</code>
 * method of this class allows to parse and compile the definition
 * <code>functionDefinition</code> of a function, that must be made of the
 * following elements :
 * <UL>
 * <LI>The <i>function</i> name and its parameters must be strings validated by
 * the <code>isValidIdentifier ()</code> method of the syntax of the parser.
 ...
 * <code>getConditionPartKey ()</code> and <code>getCommonFunctionKey ()</code>
 * methods of the syntax must return <code>null</code> for these strings. The
 * name of the function mustn't also be a function already defined by the syntax
 * (the <code>getFunction ()</code> method of the syntax must return
 * <code>null</code> for the name of the function).</LI>
 * <LI>The <i>list of parameters</i> may contain as many parameters as needed by
 * the expression, separated by the character returned by the
 * <code>getParameterSeparator ()</code> method of the syntax. This list may be
 * empty and must be bracketed between the characters returned by
 * <code>getOpeningBracket ()</code> and <code>getClosingBracket ()</code>
 * methods of the syntax.</LI>
 * <LI>The declaration of the function and its parameters must be followed by
 * the operator of assignment returned by the
 * <code>getAssignmentOperator ()</code> method of the syntax.</LI>
 * <LI>Following the operator of assignment is the expression of the function
 * that describes the operations to apply to the parameters, using the following
 * syntactic elements :
 * <UL>
 * <LI>literals (numbers, quoted strings) accepted by the
 * <code>getLiteral ()</code> method of the syntax</LI>
 * <LI>constants (PI true false ...) accepted by the
 * <code>getConstantKey ()</code> method of the syntax</LI>
 * <LI>unary operators(- ! ...) accepted by the
 * <code>getUnaryOperatorKey ()</code> method of the syntax</LI>
 * <LI>binary operators (- + / ...) accepted by the
 * <code>getBinaryOperatorKey ()</code> method of the syntax and whose priority
 * is returned by the <code>getBinaryOperatorPriority ()</code> method</LI>
 * <LI>conditions whose parts (? :) are accepted by the
 * <code>getConditionPartKey ()</code> method of the syntax</LI>
 * <LI>common functions (log sin ...) accepted by the
 * <code>getCommonFunctionKey ()</code> method of the syntax</LI>
 * <LI>other functions accepted by the <code>getFunction ()</code> method of the
 * syntax. Parameters passed to functions must be bracketed between the
 * characters returned by <code>getOpeningBracket ()</code> and
 * <code>getClosingBracket ()</code> methods of the syntax, and separated by the
 * character returned by the <code>getParameterSeparator ()</code> method. These
 * functions are instances of classes implementing the <code>Function</code>
 * interface. They may be classes written in Java or functions compiled
 * previously with a parser (the <code>CompiledFunction</code> class implements
 * <code>Function</code>).</LI>
 * </UL>
 * As the function may call itself recursively or call functions compiled
 * previously, users may create any kind of computer operations (in the limit of
 * the stack). Binary operators processed by this parser are infixed, unary
 * operators and functions are prefixed. Some parts of the expression may be
 * bracketed between the characters returned by
 * <code>getOpeningBracket ()</code> and <code>getClosingBracket ()</code>
 * methods.</LI>
 * </UL>
 * 
 ...
 * it throws a <code>CompilationException</code> exception that describe the
 * found error (syntax error or other error).<br>
 * Once a funtion is compiled, the <code>computeFunction ()</code> methods
 * available for the returned object allow to compute its value according to the
 * value of the parameters passed to them.
 * 
 * <P>
 ...
 * 
 * <PRE>
 * // Create a parser with the syntax <code>PascalSyntax</code>
 ...
 * }
 * double r4 = f4.<b>computeFunction</b> (new double [] {1, 3, 3});
 * </PRE>
 * 
 * </BLOCKQUOTE>
 * 
...
public class FunctionParser extends Parser
{
     ...
     * returns the instance of <code>CompiledFunction</code> matching the
     * definition. This method builds a tree of type <code>ExpressionNode</code>
     * matching the expression of the function. This tree is used to compute the
     * value of a function at interpretation time.
     * 
     * @param functionDefinition
     *            the definition of the function.
     * @return an instance of <code>CompiledFunction</code>, with which values
     *         can be computed.
     * @throws CompilationException
     *             if the syntax of the function is incorrect or if some
     *             elements (literals, constants, operators, functions,...) of
     *             its expression are not accepted by the syntax of the parser.
     */
    public CompiledFunction compileFunction(String functionDefinition)
            throws CompilationException
    {
        if (syntax.getAssignmentOperator() == null
                || syntax.getAssignmentOperator().length() == 0)
            throw new IllegalArgumentException(
                    "No assignment operator in syntax");

        String functionName = parseFunctionName(functionDefinition);
        String[] parametersArray = parseFunctionParametersName(
                functionDefinition, functionName);

        

        // Assign operator already checked with parameters search
        int assignmentIndex = functionDefinition.indexOf(syntax
                .getAssignmentOperator());

        CompiledFunction function = new CompiledFunction(functionDefinition, functionName, parametersArray, null);
        ExpressionNode expressionTree = (ExpressionNode) parseExpression(
                functionDefinition, assignmentIndex
                        + syntax.getAssignmentOperator().length(), function);
        function.setExpressionTree(expressionTree);
        return function;
    }

    // Search the function name in functionDefinition, checks it and returns it
    private String parseFunctionName(String functionDefinition)
            throws CompilationException
    {
        StringTokenizer rechStr = new StringTokenizer(functionDefinition,
                (syntax.getWhiteSpaceCharacters() != null ? syntax
        ...
        if (!rechStr.hasMoreTokens())
            throw new CompilationException(
                    CompilationException.INVALID_FUNCTION_NAME, 0);

        String functionName = rechStr.nextToken();
        // Checks if function characters are valid
        if (!syntax.isValidIdentifier(functionName))
            throw new CompilationException(
                    CompilationException.INVALID_FUNCTION_NAME, 0, functionName);

        // Checks if the function isn't a reserved word
        if (isReservedWord(functionName))
            throw new CompilationException(CompilationException.RESERVED_WORD,
                    0, functionName);

        // Checks if the function name doesn't already exist
        // if (syntax.getFunction (functionName) != null)
        // throw new CompilationException
        // (CompilationException.FUNCTION_NAME_ALREADY_EXISTS, 0, functionName);

        return functionName;
    }

    // Search the function parameters name in functionDefinition, checks then
    // and
    // returns them
            ...
            String functionName) throws CompilationException
    {
        ...
        int index = functionDefinition.indexOf(functionName)
                + functionName.length();
        StringTokenizer rechStr = new StringTokenizer(
                functionDefinition.substring(index), whiteSpaces);
        if (!rechStr.hasMoreTokens()
                || rechStr.nextToken().indexOf(syntax.getOpeningBracket()) != 0)
            throw new CompilationException(
                    CompilationException.OPENING_BRACKET_EXPECTED, index);

        int openingBracketIndex = functionDefinition.indexOf(syntax
                .getOpeningBracket());
        int closingBracketIndex;

        // Syntax error if no closing bracket followed with an assign operator
        if ((closingBracketIndex = functionDefinition.indexOf(
                syntax.getClosingBracket(), openingBracketIndex + 1)) == -1)
            throw new CompilationException(
                    CompilationException.CLOSING_BRACKET_EXPECTED,
                    openingBracketIndex + 1);

        rechStr = new StringTokenizer(
                functionDefinition.substring(closingBracketIndex + 1),
                whiteSpaces);
        if (!rechStr.hasMoreTokens()
                || !rechStr.nextToken().startsWith(
                        syntax.getAssignmentOperator()))
            throw new CompilationException(
                    CompilationException.ASSIGN_OPERATOR_EXPECTED,
        ...
        {
            rechStr = new StringTokenizer(functionDefinition.substring(
                    openingBracketIndex + 1, closingBracketIndex),
                    whiteSpaces
                            + (syntax.getParameterSeparator() != 0 ? String
                                    .valueOf(syntax.getParameterSeparator())
                                    : ""));
            ...
            else
            {
                ...
                for (int i = 0; rechStr.hasMoreTokens(); i++)
                {
                    ...
                    if (!syntax.isValidIdentifier(parameter))
                        throw new CompilationException(
                                CompilationException.INVALID_PARAMETER_NAME,
                                definitionIndex, parameter);

                    // Checks if parameter isn't a reserved word
                    // In case the syntax supports short cuts, parameters can't
                    // have the
                    // same
                    // name as any function of the syntax (otherwise, in
                    // expressions of
                    // the type
                    // functionOrParam (...), the parser couldn't always make
                    // its choice
                    // between a call to the
                    // function functionOrParam and an implicit multiplication
                    // of the
                    // parameter
                    // functionOrParam with a bracketed expression)
                    if (isReservedWord(parameter)
                            || (syntax.isShortSyntax() && (syntax
                                    .getFunction(parameter) != null || compare(
                                    parameter, functionName,
                                    syntax.isCaseSensitive()))))
                        throw new CompilationException(
                                CompilationException.RESERVED_WORD,
                                definitionIndex, parameter);

                    // Checks if parameter doens't already exist in function
                    // parameter
                    // list
                    for (int j = 0; j < parameters.size(); j++)
                        if (compare(parameter,
                                (String) parameters.elementAt(j),
                                syntax.isCaseSensitive()))
                            throw new CompilationException(
                                    CompilationException.DUPLICATED_PARAMETER_NAME,
                                    definitionIndex, parameter);

                    parameters.addElement(parameter);
                    definitionIndex = functionDefinition.indexOf(
                            syntax.getParameterSeparator(), definitionIndex) + 1;
                }
            }
        }
     ...
     * <code>expressionStartIndex</code> and returns its
     * <code>ExpressionNode</code> matching tree. This tree is built with the
     * automaton implemented in the <code>parseExpression ()</code> method of
     * the <code>Parser</code> class using the syntax graph and the transition
     * graph of <code>FunctionParser</code>.
     * 
     * @param functionDefinition
     *            the string to parse.
     * @param definitionIndex
     *            the index from which the parsing starts.
     * @param parserData
     *            optional data.
     * @return the expression tree of the expression.
     * @throws CompilationException
     *             if the parsed expression doesn't respect the syntax of this
     *             parser.
     * @see com.eteks.parser.Parser#parseExpression
     */
    protected Object parseExpression(String functionDefinition,
            int expressionStartIndex, Object parserData)
            throws CompilationException
    {
        ElemFct elem = (ElemFct) parseExpression(functionDefinition,
                expressionStartIndex, new StackFct(), parserData);
        return elem.expression;
    }

    /**
     * Returns <code>true</code> if <code>word</code> is a reserved word of the
     * syntax used by this parser. A reserved word might be a literal, a
     * constant, an unary or binary operator, a condition part or a common
     * function and can't be used as a the name of a function or its parameters.
     * 
     * @param word
     *            the string to check.
    ...
    /**
     * Returns <code>true</code> if the two string <code>s1</code> and
     * <code>s2</code> are equal, ignoring the case of characters if
     * <code>caseSensitive</code> is <code>false</code>.
     * 
     * @param string1
     *            first string.
     * @param string2
     ...
     * @param caseSensitive
     *            case sensitivity.
     * @return the result of the <code>equals ()</code> method if
     *         <code>caseSensitive</code> is is equal to <code>true</code>,
     *         otherwise returns the result of the
     *         <code>equalsIgnoreCase ()</code> method.
     */
    protected static boolean compare(String string1, String string2,
    ...
    /**
     * Returns the lexical found in the definition
     * <code>functionDefinition</code> beginning at the character index
     * <code>definitionIndex</code>. This method may be overriden in subclasses
     * to parse additional kind of parameters and return
     * <code>LEXICAL_PARAMETER</code>. In that case, the
     * <code>getParameterNode ()</code> method must be overriden to return the
     * expression node matching the additional parameters.
     * 
     * @param functionDefinition
     *            the string to parse.
     * @param definitionIndex
     ...
     * @param parserData
     *            used to store the current instance of
     *            <code>CompiledFunction</code>.
     * @return a lexical with a code among <code>LEXICAL_VOID</code>,
     *         <code>LEXICAL_WHITE_SPACE</code>, <code>LEXICAL_LITERAL</code>,
     *         <code>LEXICAL_UNARY_OPERATOR</code>,
     *         <code>LEXICAL_BINARY_OPERATOR</code>,
     *         <code>LEXICAL_COMMON_FUNCTION</code>,
     *         <code>LEXICAL_OPENING_BRACKET</code>,
     *         <code>LEXICAL_CLOSING_BRACKET</code>,
     *         <code>LEXICAL_SYNONYMOUS_OPERATOR</code>,
     *         <code>LEXICAL_CONSTANT</code>, <code>LEXICAL_PARAMETER</code>,
     *         <code>LEXICAL_FUNCTION</code>,
     *         <code>LEXICAL_PARAMETER_SEPARATOR</code>, <code>LEXICAL_IF</code>
     *         , <code>LEXICAL_THEN</code>, <code>LEXICAL_ELSE</code>.
     * @throws CompilationException
     *             if this method doesn't recognize any expression lexical at
     *             index <code>definitionIndex</code>.
    ...
    protected Lexical getLexical(String functionDefinition,
            int definitionIndex, Object parserData) throws CompilationException
    {
        ...
        while (definitionIndex < functionDefinition.length()
                ...
                && syntax.getWhiteSpaceCharacters().indexOf(
                        functionDefinition.charAt(definitionIndex)) != -1)
            extractedLexical.append(functionDefinition
                    .charAt(definitionIndex++));

        if (extractedLexical.length() > 0)
            return new Lexical(LEXICAL_WHITE_SPACE,
                    extractedLexical.toString(), null);
        ...
        if (c == syntax.getParameterSeparator())
            return new Lexical(LEXICAL_PARAMETER_SEPARATOR, String.valueOf(c),
                    null);

        String subExpression = functionDefinition.substring(definitionIndex);
        // v1.0.1 Moved getLiteral () to the end to give an higher priority to
        // operators

        String token = null;
        StringTokenizer tokens = new StringTokenizer(subExpression,
                syntax.getDelimiters());

        if (tokens.hasMoreTokens())
            token = tokens.nextToken();
        if (token == null || !subExpression.startsWith(token))
        {
            ...
            for (int i = 0; i < subExpression.length(); i++)
            {
                ...
                if (syntax.getUnaryOperatorKey(operator) != null
                    ...
                    token = operator; // Don't break the loop now, some
                                      // operators may be
                                      // longer
            ...
            if (token == null)
                throw new CompilationException(
                        CompilationException.SYNTAX_ERROR, definitionIndex);
        } else
        {
            ...
            if (parserData instanceof CompiledFunction)
            {
                ...
                for (int i = 0; i < parameters.length; i++)
                    if (compare(token, parameters[i], syntax.isCaseSensitive()))
                        return new Lexical(LEXICAL_PARAMETER, token,
                                new Integer(i));
            }
            ...
            if (parserData instanceof CompiledFunction
                    && compare(token,
                            ((CompiledFunction) parserData).getName(),
                            syntax.isCaseSensitive()))
        ...
        // v1.0.1 Moved getLiteral () to the end to give an higher priority to
        // operators
        Object lexicalValue = syntax
                .getLiteral(subExpression, extractedLexical);
        if (lexicalValue != null)
            return new Lexical(LEXICAL_LITERAL, extractedLexical.toString(),
                    lexicalValue);

        if (token.equals("DEFAULT"))
        {
            //throw new CompilationException(
            //        CompilationException.UNKOWN_IDENTIFIER, definitionIndex);            
            return new Lexical(LEXICAL_DEFAULT_FIELD, "DEFAULT", new UserTypeInstance.DefaultParam());
        }
        // No known lexical was found, throw an exception
        throw new CompilationException(
                syntax.isValidIdentifier(token) ? CompilationException.UNKOWN_IDENTIFIER
                        : CompilationException.SYNTAX_ERROR, definitionIndex,
                token);
    }

    /**
     * Returns the node bound to <code>graphNode</code> with the link
     * <code>lexical</code> in the syntactic graph of the calculator.<br>
     * <code>lexical</code>, <code>expressionStack</code> and
     * <code>parserData</code> are used to create the tree of the expression of
     * the current parsed function.
     * 
     * @param graphNode
     *            a node in the syntactic graph. First node is equal to
     *            <code>NODE_START</code>.
     * @param lexical
     *            the last parsed lexical.
     * @param definitionIndex
     *            the current index of parsing.
     * @param expressionStack
     *            stack used to accumulate the operators of different priority
     *            or <code>null</code>.
     * @param parserData
     *            contains some extra information used by the parser.
     * @return the node bound to <code>graphNode</code> with the link
     *         <code>lexical</code>. Returns <code>NODE_END</code> if the
     *         automaton must stop parsing.
     * @throws CompilationException
     *             if no link of type <code>lexical</code> starts from
     *             <code>graphNode</code> or another kind of error of
     *             compilation occures.
     */
    protected int getBoundNode(int graphNode, Lexical lexical,
            int definitionIndex, Stack expressionStack, Object parserData)
            throws CompilationException
    {
        int boundNode = (syntax.isShortSyntax() ? shortSyntaxGraph
                : syntaxGraph)[lexical.getCode()][graphNode];
        if (boundNode == -1)
            throw new CompilationException(CompilationException.SYNTAX_ERROR,
                    definitionIndex);

        // Execute the transition that compiles the expression
        executeTransition(transitionGraph[lexical.getCode()][graphNode],
                definitionIndex, lexical, expressionStack, parserData);

        return boundNode == N_END ? Parser.NODE_END : boundNode;
    }

    // Lexical types
    /**
     * Lexical code returned by <code>getLexical ()</code> at the end of parsed
     * string.
     */
    protected final static int LEXICAL_VOID = 0; // Empty string
    /**
     * Lexical code returned by <code>getLexical ()</code> for a sequence of
     * white spaces.
     * 
    ...
    /**
     * Lexical code returned by <code>getLexical ()</code> for a literal
    ...
    /**
     * Lexical code returned by <code>getLexical ()</code> for an unary
     * operator.
     * 
    ...
    /**
     * Lexical code returned by <code>getLexical ()</code> for a binary
     * operator.
     * 
    ...
    /**
     * Lexical code returned by <code>getLexical ()</code> for a common
     * function.
     * 
    ...
    /**
     * Lexical code returned by <code>getLexical ()</code> for an opening
     * bracket.
     * 
    ...
    /**
     * Lexical code returned by <code>getLexical ()</code> for a closing
     * bracket.
     * 
    ...
    /**
     * Lexical code returned by <code>getLexical ()</code> for synonymous unary
    ...
    /**
     * Lexical code returned by <code>getLexical ()</code> for an IF condition
     * part.
     * 
    ...
    /**
     * Lexical code returned by <code>getLexical ()</code> for a THEN condition
     * part.
     * 
     * @see com.eteks.parser.Syntax#getConditionPartKey
     */
    ...
    /**
     * Lexical code returned by <code>getLexical ()</code> for an ELSE condition
     * part.
     * 
     * @see com.eteks.parser.Syntax#getConditionPartKey
     */
    ...
    protected final static int LEXICAL_DEFAULT_FIELD = 16;
    ...
    // v1.0.2 : Added nodes (N_MULT, LEXICAL_VOID), (N_MULT,
    // LEXICAL_BINARY_OPERATOR), (N_MULT, LEXICAL_UNARY_OPERATOR), (N_MULT,
    // LEXICAL_SYNONYMOUS_OPERATOR),
    // (N_MULT, LEXICAL_CLOSING_BRACKET), (N_MULT, LEXICAL_PARAMETER_SEPARATOR),
    // (N_MULT, LEXICAL_THEN), (N_MULT, LEXICAL_ELSE)
    // to correct the bug where a string using useless spaces like "2 * 2"
    // throws
    // a syntax error (not "2*2" or "2 2")
    // N_START N_CL_BR N_C_FCT N_OP_IF N_U_FCT N_OP_BR N_UN_OP N_MULT
    {
            { -1, N_END, -1, -1, -1, -1, -1, N_END }, // LEXICAL_VOID
            { N_START, N_MULT, N_OP_BR, N_OP_IF, N_U_FCT, N_OP_BR, N_UN_OP,
                    N_MULT }, // LEXICAL_WHITE_SPACE
            ...
            { N_OP_BR, N_OP_BR, N_OP_BR, N_OP_BR, N_OP_BR, N_OP_BR, N_OP_BR,
                    N_OP_BR }, // LEXICAL_OPENING_BRACKET
    ...
    { -1, N_END, -1, -1, -1, -1, -1 }, // LEXICAL_VOID
            ...
            { N_CL_BR, -1, -1, N_CL_BR, -1, N_CL_BR, N_CL_BR } }; // LEXICAL_DEFAULT_VALUE
    ...
    // N_START N_CL_BR N_C_FCT N_OP_IF N_U_FCT N_OP_BR N_UN_OP N_MULT
    {
            ...
            { ADD_TERNARY_IF_LITERAL, ADD_MULT_LITERAL, ADD_LITERAL,
                    ADD_LITERAL, 0, ADD_TERNARY_IF_LITERAL, ADD_LITERAL,
                    ADD_MULT_LITERAL },
            { ADD_TERNARY_IF_UNARY_OP, 0, ADD_UNARY_OP, ADD_UNARY_OP, 0,
                    ADD_TERNARY_IF_UNARY_OP, ADD_UNARY_OP, ADD_MULT_UNARY_OP },
            { 0, ADD_BINARY_OP, 0, 0, 0, 0, 0, ADD_BINARY_OP },
            { ADD_TERNARY_IF_COMMON_FCT, ADD_MULT_COMMON_FCT, ADD_COMMON_FCT,
                    ADD_COMMON_FCT, 0, ADD_TERNARY_IF_COMMON_FCT,
                    ADD_COMMON_FCT, ADD_MULT_COMMON_FCT },
            { ADD_TERNARY_IF_OPEN_BRACKET, ADD_MULT_OPEN_BRACKET,
                    ADD_OPEN_BRACKET, ADD_OPEN_BRACKET, 0,
                    ADD_TERNARY_IF_OPEN_BRACKET, ADD_OPEN_BRACKET,
                    ADD_MULT_OPEN_BRACKET },
            { 0, CHECK_CLOSE_BRACKET, 0, 0, 0, CHECK_OPEN_CLOSE_BRACKETS, 0,
                    CHECK_CLOSE_BRACKET },
            { ADD_TERNARY_IF_UNARY_OP, ADD_BINARY_OP, ADD_UNARY_OP,
                    ADD_UNARY_OP, 0, ADD_TERNARY_IF_UNARY_OP, ADD_UNARY_OP,
                    ADD_BINARY_OP },
            { ADD_TERNARY_IF_CONSTANT, ADD_MULT_CONSTANT, ADD_CONSTANT,
                    ADD_CONSTANT, 0, ADD_TERNARY_IF_CONSTANT, ADD_CONSTANT,
                    ADD_MULT_CONSTANT },
            { ADD_TERNARY_IF_PARAMETER, ADD_MULT_PARAMETER, ADD_PARAMETER,
                    ADD_PARAMETER, 0, ADD_TERNARY_IF_PARAMETER, ADD_PARAMETER,
                    ADD_MULT_PARAMETER },
            { ADD_TERNARY_IF_USER_FCT, ADD_MULT_USER_FCT, ADD_USER_FCT,
                    ADD_USER_FCT, 0, ADD_TERNARY_IF_USER_FCT, ADD_USER_FCT,
                    ADD_MULT_USER_FCT },
            { 0, CHECK_PARAMETER_SEPARATOR, 0, 0, 0, 0, 0,
    ...
    private void executeTransition(byte transition, int definitionIndex,
            Lexical lexical, Stack expressionStack, Object parserData)
            throws CompilationException
    {
        switch (transition)
        {
            ...
            case ADD_MULT_CONSTANT:
                ...
                break;
            case ADD_PARAMETER:
                addParameter(definitionIndex, lexical, expressionStack,
                        parserData);
                break;
    ...
    {
        Object binaryOperatorKey = binaryOperatorLexical.getValue() != null ? binaryOperatorLexical
                .getValue() : syntax.getBinaryOperatorKey(binaryOperatorLexical
                .getExtractedString());
    ...
    private void addBinaryOperator(Object binaryOperatorKey,
            Stack expressionStack)
    {
        int binaryOperatorPriority = syntax
                .getBinaryOperatorPriority(binaryOperatorKey);

        // Stack can't be empty : the element at top is the first operand
        while (expressionStack.size() > 1
                && ((ElemFct) expressionStack
                        .elementAt(expressionStack.size() - 2)).priority >= binaryOperatorPriority)
            expressionStack.pop();
    ...
    private void addLiteral(Lexical literalLexical, Stack expressionStack)
    {
        ExpressionNode node = new LiteralNode(literalLexical.getValue());
        expressionStack.push(new ElemFct(PRIORITY_OPERAND, node));
    }

    private void addConstant(Lexical constantLexical, Stack expressionStack)
    {
        ExpressionNode node = new ConstantNode(constantLexical.getValue());
        expressionStack.push(new ElemFct(PRIORITY_OPERAND, node));
    }

    private void addParameter(int definitionIndex, Lexical parameterLexical,
            Stack expressionStack, Object parserData)
            throws CompilationException
    {
    ...
    /**
     * Returns the expression node matching the extracted lexical
     * <code>parameterLexical</code>.
     * 
     * @param parameterLexical
     *            the lexical extracted as a parameter.
     * @param parserData
     *            optional user data.
     * @return an expression node or <code>null</code> if <code>parameter</code>
     *         is unknown. This shouldn't happen unless
     *         <code>getLexical ()</code> was overriden to return
            ...
            Object parserData)
    {
        if (parserData instanceof CompiledFunction)
            return new FunctionParameterNode(
                    ((Integer) parameterLexical.getValue()).intValue());
    ...
    private void addCommonFunction(Lexical commonFunctionLexical,
            Stack expressionStack)
    {
        ExpressionNode node = new CommonFunctionNode(
                commonFunctionLexical.getValue());
        expressionStack.push(new ElemFct(PRIORITY_COMMON_FUNCTION, node));
    }

    // Called when an opening bracket not following a function name is
    // encountered
    ...
    private void addUnaryOperator(Lexical unaryOperatorLexical,
            Stack expressionStack)
    {
        Object unaryOperatorKey = unaryOperatorLexical.getValue() != null ? unaryOperatorLexical
                .getValue() : syntax.getUnaryOperatorKey(unaryOperatorLexical
                .getExtractedString());
    ...
    private void addFunction(Lexical functionLexical, Stack expressionStack,
            Object parserData)
    {
    ...
    // Called when a closing bracket is encountered after a parameter or a
    // literal
    private void checkClosingBracket(int definitionIndex, Stack expressionStack)
            throws CompilationException
    {
        ElemFct stackTop = null;

        for (; !expressionStack.empty()
                && (stackTop = (ElemFct) expressionStack.peek()).priority > PRIORITY_OPENING_BRACKET; expressionStack
                .pop())
            if (stackTop.priority == PRIORITY_IF)
                throw new CompilationException(
                        CompilationException.THEN_OPERATOR_EXPECTED,
                        definitionIndex);
            else if (stackTop.priority == PRIORITY_THEN)
                throw new CompilationException(
                        CompilationException.ELSE_OPERATOR_EXPECTED,
                        definitionIndex);

        if (expressionStack.empty())
            throw new CompilationException(
                    CompilationException.CLOSING_BRACKET_WITHOUT_OPENING_BRACKET,
                    definitionIndex);

        if (stackTop.priority == PRIORITY_USER_FUNCTION)
        {
            FunctionNode functionNode = (FunctionNode) stackTop.expression;
            if (!functionNode.getFunction().isValidParameterCount(
                    functionNode.getParameterCount()))
                throw new CompilationException(
                        CompilationException.MISSING_PARAMETERS_IN_FUNCTION_CALL,
                        definitionIndex);
        }
    ...
    // Called when a sequence () is encountered to check if a user function may
    // be
            ...
            Stack expressionStack) throws CompilationException
    {
        ...
        for (; !expressionStack.empty()
                && (stackTop = (ElemFct) expressionStack.peek()).priority != PRIORITY_USER_FUNCTION; expressionStack
                .pop())
        ...
        if (functionNode.getFunction() instanceof CompiledFunction
                && ((CompiledFunction) functionNode.getFunction())
                        .getParameterCount() <= functionNode
                        .getParameterCount())
            throw new CompilationException(
                    CompilationException.CLOSING_BRACKET_EXPECTED,
                    definitionIndex);
    ...
    private void checkThen(int definitionIndex, Stack expressionStack)
            throws CompilationException
    {
        ElemFct stackTop = null;
        ...
        for (; !expressionStack.empty()
                ...
                && stackTop.priority != PRIORITY_TERNARY_IF; expressionStack
                .pop())
            if (stackTop.priority <= PRIORITY_OPENING_BRACKET)
                throw new CompilationException(
                        CompilationException.SYNTAX_ERROR, definitionIndex);
        ...
        if (expressionStack.empty())
            throw new CompilationException(
                    CompilationException.THEN_OPERATOR_WITHOUT_IF_OPERATOR,
                    definitionIndex);
    ...
    private void checkElse(int definitionIndex, Stack expressionStack)
            throws CompilationException
    {
        ElemFct stackTop = null;

        for (; !expressionStack.empty()
                && (stackTop = (ElemFct) expressionStack.peek()).priority != PRIORITY_THEN; expressionStack
                .pop())
            if (stackTop.priority <= PRIORITY_OPENING_BRACKET)
                throw new CompilationException(
                        CompilationException.SYNTAX_ERROR, definitionIndex);
        ...
        if (expressionStack.empty())
            throw new CompilationException(
                    CompilationException.ELSE_OPERATOR_WITHOUT_IF_THEN_OPERATORS,
                    definitionIndex);

        stackTop.priority = PRIORITY_ELSE;
    }

    private void checkEnd(int definitionIndex, Stack expressionStack)
            throws CompilationException
    {
        ElemFct stackTop = null;
        ...
        // Push back the expression result to retrieve it in the parseExpression
        // ()
        // method
    ...
    private static class StackFct extends Stack
    {
        public Object pop()
        {
            ...
            if (!empty() && topElement.expression != null)
            {
                ElemFct element = (ElemFct) peek();
                if (element.expression instanceof ParameterizedNode)
                    ((ParameterizedNode) element.expression)
                            .addParameter(topElement.expression);
                else
