 ...
 * @since Jeks 1.0
 */
public class BinaryOperatorNode extends ConstantNode implements
        ParameterizedNode
{
    ...
    {
        return firstOperand != null ? (secondOperand != null ? 2 : 1) : 0;
    }
    ...
    /**
     * Returns the value returned by the <code>getBinaryOperatorValue ()</code>
     * method of <code>interpreter</code> with the key of this operator and the
     * computed value of its operands as parameters.
     * 
     * @param interpreter
     *            runtime interpreter.
     * @param parametersValue
     *            the value of parameters passed to compute a compiled function.
     * @return the computed value of this operator. The type of the returned
    ...
    {
        Object operand1 = firstOperand.computeExpression(interpreter,
                parametersValue);
        Object operand2 = secondOperand.computeExpression(interpreter,
                parametersValue);
    ...
    public double computeExpression(double[] parametersValue)
    {
        ...
        else if (binaryOperatorKey.equals(Syntax.OPERATOR_MODULO))
        {
            ...
            return number1 != FALSE_DOUBLE || number2 != FALSE_DOUBLE ? TRUE_DOUBLE
                    : FALSE_DOUBLE;
            ...
            return number1 != FALSE_DOUBLE && number2 == FALSE_DOUBLE
                    || number1 == FALSE_DOUBLE && number2 != FALSE_DOUBLE ? TRUE_DOUBLE
                    : FALSE_DOUBLE;
        else if (Math.floor(number1) != number1)
            throw new IllegalArgumentException("Operand " + number1
                    + " of bit operator not an integer");
        else if (Math.floor(number2) != number2)
            throw new IllegalArgumentException("Operand " + number2
                    + " of bit operator not an integer");
